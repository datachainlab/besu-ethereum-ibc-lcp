...
make -C tests/e2e/cases/besu2eth network
...
./tests/e2e/cases/besu2eth/scripts/gen_rly_config.sh

make -C tests/e2e/cases/besu2eth setup handshake
...
${LCP_BIN} --log_level=info service start --enclave=${ENCLAVE_PATH} --address=127.0.0.1:50051 --threads=2 &
LCP_PID=$!
make -C tests/e2e/cases/besu2eth restore

make -C tests/e2e/cases/besu2eth test
make -C tests/e2e/cases/besu2eth network-down
kill $LCP_PID
